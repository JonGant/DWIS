...
from phantomas.mr_simul import image_formation
...
def signal_function(bvec, bval, dt_data, roi):
    gm_dt = np.array([[.0008, 0, 0], [0, .0008, 0], [0, 0, .0008]])
    ...
    if n_fibers >= 1:
        ...
        if iso_on:
            signal = (1/(n_fibers + 1)) * (isotropic_signal + sum(signal_terms))
        else:
            signal = (1 / n_fibers) * sum(signal_terms)
    elif roi > 0:
...
def simulate_dwi_calc(xsize, ysize, zsize, bvalue, dirpershell, dt_data, filename, rois):
...
def circular_fit(xsize, ysize, zsize, points, eigenvalues, fiber_radius):
    ...
    n_steps = 10 * int((xsize + ysize + zsize) / 3)
...
def linear_fit(xsize, ysize, zsize, points, eigenvalues, radius):
    ...
    step_size = 0.25
    t = step_size
    # find the voxels within the radius of the line defined by the direction vector
    while t <= range_t:
        line_point = linear_eq(t)
        linear_d = np.dot(direction, line_point)
        for i in range(xsize):
            for j in range(ysize):
                for k in range(zsize):
                    voxel = np.array([i, j, k])
                    # define line perpendicular to plane going through the given point
                    s = linear_d - np.dot(direction, voxel)
                    closest_point_on_plane = voxel + s * direction
                    if np.linalg.norm(voxel - closest_point_on_plane) <= 1:
                        # find the point on the line which minimizes the distance to the test point
                        line_min_dist = np.linalg.norm(line_point - closest_point_on_plane)
                        if line_min_dist <= radius:
                            fiber_dts[i, j, k] = rotated_dt
        t += step_size
...
if __name__ == "__main__":
    ...
    class MainMenu(tk.Frame):
        def __init__(self, parent, controller):
            ...
            filename_lbl.grid(row=9, column=0)
                                        ...
                                        if 0 < int(xsize.get()) <= 100 and 0 < int(ysize.get()) <= 100 and 0 < int(zsize.get()) <= 100:
                                            ...
                                            while(param):
                                                ...
                                                param = simulate_dwi_calc(int(xsize.get()), int(ysize.get()), int(zsize.get()),
                                                                          bvals, dir_shell, dt_data, filename.get(), roi_list_formatted)
            ...
            for i in range(len(roi_info)):
                if i < 30:
                    ...
                    if roi_info[i][0] == "cuboid":
                        ...
                        self.manual_buttonsl[i].configure(bg=self.orig_color)
                        self.cuboid_buttons[i].configure(bg='gray')
                        self.sphere_buttons[i].configure(bg='gray')
                elif i >= 30:
                    self.roi_lbls[i].grid(row=i - 28, column=7)
    ...
    class AddFiber(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            info1 = ttk.Label(self, width=110, text="Enter a tuple x, y, z with spaces in the second entry box "
                                                    ...
                                                    "(e.g. .0010, .0003, .0003).", font=LARGE_FONT, anchor='w',
                                                    justify=tk.LEFT)
